...
This package runs in Claude Code, including the Claude Code extension for VS Code. The Lockstep VS Code extension reads the same files to render phase status, gate prompts and the audit log.

## Core Principles

1. **Gates are approvals by a named role, not by whoever types the next command.** Each phase ends by requesting a gate. The next phase refuses to start until `adlc_gate.py check` passes for the previous gate: an approval by an identity holding the required role, or a recorded waiver. This is the main difference from single-user skill packs, where invoking the next command counts as approval.
2. **Separation of duties.** The identity that initiated or authored a phase cannot approve that phase's gate. `adlc_gate.py` enforces this; never work around it.
...
4. **Tamper-evident audit.** Every gate request, decision, waiver, policy block and phase transition is appended to `adlc/.audit/audit.jsonl` through `audit_log.py`. Entries are hash-chained. Never edit or truncate that file by hand.
...
Supporting skills: /approve-gate (record a human decision), /status, /cancel.

Every phase: verify the previous gate, write its artifact, run validation and its policy pack, request its gate, update state, give a short summary with links, recommend the next command, and stop. Never auto-invoke the next phase.
...
adlc/                       # the lifecycle record for this intent (committed to the repo)
  ...
  .gates/                   # one JSON per gate decision (written by adlc_gate.py only)
...
Use clickable links for files. Summarize; do not dump.
