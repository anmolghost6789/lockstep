...
2. Order units by dependency. For each unit:
...
3. After all units are accepted: `validate_artifact.py --phase build-orchestrate` and `policy_check.py --phase build-orchestrate` (no_secrets, pii_scan, mcp_allowlist, tests_per_bolt, license_scan). `license_scan` passes only with CI evidence in `adlc/.state/evidence/license_scan.json`.

## Human Gate G3
- Approver role: `engineer`, and must not be the identity recorded as `built_by` or the requester.
- Request: `adlc_gate.py request --gate G3 --summary "<N> units, <B> bolts, <T> tests passing"`.
- Approver runs `/approve-gate G3`.

## Phase completion
4. `state.py complete --phase build-orchestrate`.
5. Summary: units, bolts, tests, pull requests, anything left dry-run, links.

## End Of Turn
G3 is waiting for an engineer. Recommend `/evaluate-validate` once approved. Stop.
