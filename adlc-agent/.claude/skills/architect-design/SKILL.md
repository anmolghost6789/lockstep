...
## Human Gate G2
- Approver role: `architect`. Separation of duties applies.
- Request: `adlc_gate.py request --gate G2 --summary "<N> units, <M> agents, <K> ADRs"`.
- Approver runs `/approve-gate G2`.
...
## End Of Turn
G2 is waiting for an architect. Recommend `/build-orchestrate` once approved. Stop.
