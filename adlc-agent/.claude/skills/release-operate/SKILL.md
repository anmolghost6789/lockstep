...
- The agent never deploys to production itself. It prepares the release; the pipeline deploys after G5, checking the gate file and its artifact hash.
...
## Human Gate G5
- Approver roles: `release_manager`; plus `security_officer` if risk is high; plus `waiver_approver` if critical. Separation of duties applies to each.
- Request: `adlc_gate.py request --gate G5 --risk <level> --summary "<release id>, <rollout %>"`.
- Approvers run `/approve-gate G5`. The gate is approved only when every required role has approved.
...
## End Of Turn
G5 is waiting for its approvers. After approval the pipeline deploys; recommend `/observe-evolve` once the observation window in the release record has elapsed. Stop.
