...
## Human Gate G6
- Approver role: `product_owner`.
- Request: `adlc_gate.py request --gate G6 --summary "<N> backlog items, next intent: BL-00X"`.
- Approval closes this cycle and authorises /discover-define on the chosen next intent.

## Phase completion
9. `state.py complete --phase observe-evolve`.
10. Summary: success criteria met and missed, top incidents, backlog size, proposed next intent, links.

## End Of Turn
G6 is waiting for a product owner. Once approved, recommend `/discover-define` with the next intent. Stop.
