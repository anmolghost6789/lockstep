...
- Approver role: `product_owner`. The requester cannot approve (separation of duties).
- Request: `python3 .claude/skills/adlc/scripts/adlc_gate.py request --gate G1 --summary "<intent in one line>"`
- The approver reviews `adlc/01-aiprs.md` and runs `/approve-gate G1` in their own session.
- Any edit to `adlc/01-aiprs.md` after approval invalidates G1 (hash check), and /architect-design will refuse to start.
...
## End Of Turn
Tell the user G1 is waiting for a product owner and that the approver runs `/approve-gate G1`. Recommend `/architect-design` once G1 is approved. Stop.
