...
export function PhaseGrid({
  ...
  return (
    <>
      <div className="relative grid gap-x-6 gap-y-10 py-6 md:grid-cols-2 xl:grid-cols-3">
          ...
          return (
            <AnimateEnter key={card.id} duration={0.55} delay={index * 0.05} className="flex flex-col">
              ...
              <button
                ...
                className={cn(
                  CARD,
                  'group flex h-full flex-col text-left transition-[border-color,box-shadow,transform] duration-200 ease-out',
              ...
              >
                <div className="flex items-start gap-3 border-b border-black/[0.06] px-4 py-3.5 dark:border-white/[0.07]">
                  <span className="grid size-8 shrink-0 place-items-center rounded-lg bg-neutral-100 text-neutral-700 dark:bg-white/[0.07] dark:text-neutral-300">
                ...
                <div className="flex items-center justify-between gap-3 border-t border-black/[0.06] px-4 py-2.5 dark:border-white/[0.07]">
                  <span className="flex items-center gap-1.5 font-inter text-[12px] font-medium text-neutral-800 dark:text-neutral-200">
                    <UserCheck className="size-3.5 text-[#f9452d] dark:text-[#E1F435]" />
                    {card.gate}
                  </span>
                  <span className="flex items-center gap-1 font-mono text-[11px] text-neutral-500 transition-colors group-hover:text-[#f9452d] dark:group-hover:text-[#E1F435]">
                    {skill?.command}
...
function MarkdownFile({ source }: { source: string }) {
...
function CodeFile({ source }: { source: string }) {
